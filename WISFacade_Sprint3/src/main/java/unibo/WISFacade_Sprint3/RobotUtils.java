...
public class RobotUtils {
    ...
    public static IApplMessage moveAril(String robotName, String cmd  ) {
        ...
        switch( cmd ) {
            ...
            case "p" : return CommUtils.buildRequest(applName, "step", "step("+steptime +")", robotName);
...
*/
    ...
    public static String sendApplMsg(IApplMessage msg){
        try {
             CommUtils.outblue("RobotUtilsss | sendApplMsg msg:" + msg + " conn=" + tcpconn);
            Interaction conn = (tcpconn != null) ? tcpconn  : coapconn;

            if( msg.isRequest() ){
                String answer = conn.request( msg.toString() );
                CommUtils.outblue("RobotUtils | sendApplMsg req answer:" + answer  );
                return answer;
            }
            else
            ...
            CommUtils.outred("RobotUtils | sendApplMsg on:" + tcpconn + " ERROR:"+e.getMessage());
    ...
    public static String sendPlanMsg(String plan, String steptime ){
        try {
        	if( plan.isEmpty()) {
        		return "RobotUtils:no plan ERROR";
        	}
            String msg = ""+ CommUtils.buildRequest(applName,
                    "doplan", "doplan("+plan+ "," + steptime + ")", "basicrobot");
            Interaction conn = (tcpconn != null) ? tcpconn  : coapconn;
                ...
                new Thread(){
                    public void run(){
                        try {
                            ...
                            CommUtils.outred("RobotUtils sendPlanMsg |  alarm msg:" + msg + " conn=" + conn);
                            conn.forward( msg );
                            withalarm = false;
                        } catch ( Exception e) {
                            e.printStackTrace();
            ...
            CommUtils.outmagenta("RobotUtils  sendPlanMsg | answer=" + answer);
            withalarm = false;
            ...
            return "RobotUtils sendPlanMsg | ERROR";
        }
    }
